...
class LexiFinderCLI:
    ...
    def get_models_directory():
        """Get the directory where external/downloaded models are stored.

        Always placed in the user's home directory so that the path is
        writable even when the application runs from a read-only location
        (e.g. a Program Files folder or an MSIX package).
        """
        models_dir = os.path.join(os.path.expanduser("~"), ".lexifinder_models")
        os.makedirs(models_dir, exist_ok=True)
        return models_dir

    @staticmethod
    def _is_model_available(model_name: str) -> bool:
        """Check whether a spaCy model can be loaded, regardless of context.

        Checks (in order):
        1. Standard importable Python package (normal script execution).
        2. Bundled directory inside sys._MEIPASS (PyInstaller frozen exe).
        3. Downloaded directory inside the user models directory.

        Args:
            model_name: spaCy model name (e.g. "en_core_web_md")

        Returns:
            True if the model can be found/loaded.
        """
        # 1. Registered Python package
        try:
        ...
        # 2. Bundled in MEIPASS (frozen only)
        if getattr(sys, 'frozen', False):
            if os.path.isdir(os.path.join(sys._MEIPASS, model_name)):
                return True

        # 3. User models directory
        user_path = os.path.join(LexiFinderCLI.get_models_directory(), model_name)
    ...
    def download_model(model_name: str) -> bool:
        """Download a spaCy model using only the Python stdlib.

        Uses the spaCy compatibility matrix on GitHub to find the correct model
        version for the currently installed spaCy, then fetches the wheel
        directly from GitHub Releases and extracts it into
        ``~/.lexifinder_models/<model_name>/``.

        No external Python interpreter, no pip subprocess, no external tools.
        Works in every execution context: normal script, PyInstaller exe,
        MSIX package, AppImage.

        Args:
            model_name: spaCy model name (e.g. ``"en_core_web_md"``)

        Returns:
            ``True`` if the model was downloaded and extracted successfully.
        """
        ...
        models_dir = LexiFinderCLI.get_models_directory()
        whl_path   = os.path.join(models_dir, f"_{model_name}.whl.tmp")

        try:
            # ── 1. Determine compatible model version ─────────────────────────
            # spaCy publishes a compatibility matrix at:
            #   https://raw.githubusercontent.com/explosion/spacy-models/master/compatibility.json
            # Structure: {"spacy": {"3.8.0": {"en_core_web_md": ["3.8.0"], ...}}}
            spacy_ver = spacy.__version__                           # e.g. "3.8.0"
            spacy_mm  = ".".join(spacy_ver.split(".")[:2])         # e.g. "3.8"

            print(f"\nDownloading spaCy model '{model_name}'...")
            print(f"  Installed spaCy : {spacy_ver}")
            print("  Fetching spaCy compatibility matrix...")

            compat_url = (
                "https://raw.githubusercontent.com/explosion/"
                "spacy-models/master/compatibility.json"
            )
            req = urllib.request.Request(
                compat_url,
                headers={"User-Agent": f"LexiFinder/{APP_VERSION}"},
            )
            with urllib.request.urlopen(req, timeout=30) as resp:
                compat = json.loads(resp.read().decode())

            # The top-level key may be "spacy" or the version directly
            spacy_map = compat.get("spacy", compat)

            model_version = None
            for key in (spacy_ver, spacy_mm):
                versions = spacy_map.get(key, {}).get(model_name)
                if versions:
            ...
            if not model_version:
                print(
                    f"\n✗ No compatible version of '{model_name}' found "
                    f"for spaCy {spacy_ver}.\n"
                    "  Check that the model name is correct and that it "
                    "supports your spaCy version."
                )
                return False

            print(f"  Compatible model version : {model_version}")

            # ── 2. Build the GitHub Releases download URL ─────────────────────
            # URL pattern used by Explosion for all spaCy models:
            #   https://github.com/explosion/spacy-models/releases/download/
            #       {model_name}-{version}/{model_name}-{version}-py3-none-any.whl
            whl_filename = f"{model_name}-{model_version}-py3-none-any.whl"
            whl_url = (
                f"https://github.com/explosion/spacy-models/releases/download/"
                f"{model_name}-{model_version}/{whl_filename}"
            )

            # ── 3. Download the wheel with progress reporting ─────────────────
            ...
            with urllib.request.urlopen(req2, timeout=300) as src_resp,                  open(whl_path, "wb") as out_f:
                total      = int(src_resp.headers.get("Content-Length", 0) or 0)
                downloaded = 0
                while True:
                    chunk = src_resp.read(65536)   # 64 KiB
                    if not chunk:
                        break
                    out_f.write(chunk)
            ...
            if total > 0:
                print(f"  100%  ({downloaded/1_048_576:.1f} MB)")

            # ── 4. Extract the model directory from the wheel (zip) ───────────
            # spaCy wheel layout inside the zip:
            #
            #   {model_name}/
            #     {model_name}-{version}/        ← model root (has config.cfg)
            #       config.cfg
            #       meta.json
            #       vocab/
            #       ...
            #   {model_name}-{version}.dist-info/
            #
            # We find config.cfg to locate the model root, then extract every
            # file under that root, stripping the root prefix, so that
            # spacy.load(dst) works directly.
            ...
            with zipfile.ZipFile(whl_path, "r") as zf:
                ...
                # Find config.cfg – use the shallowest occurrence
                cfg_entries = sorted(
                    [n for n in names if n.endswith("/config.cfg")],
                    key=len,
                )
                if not cfg_entries:
                    # Fallback: meta.json
                    cfg_entries = sorted(
                        [n for n in names if n.endswith("/meta.json")],
                        key=len,
                    )
                if not cfg_entries:
                    print("✗ Cannot identify model root inside the wheel archive.")
                    return False

                model_root = "/".join(cfg_entries[0].split("/")[:-1]) + "/"

                for entry in names:
                    if not entry.startswith(model_root):
                        continue
                    rel = entry[len(model_root):]
                    if not rel:
                        continue
                    ...
                    if entry.endswith("/"):
                        ...
                        with zf.open(entry) as src_f, open(target, "wb") as out_f:
                            out_f.write(src_f.read())

            # ── 5. Remove temporary wheel file ────────────────────────────────
            try:
                os.remove(whl_path)
            except OSError:
                pass
            ...
            # ── 6. Verify ─────────────────────────────────────────────────────
            if os.path.isfile(os.path.join(dst, "config.cfg")):
                print(
                ...
                print(
                    "✗ Extraction finished but config.cfg was not found.\n"
                    "  The model may be unusable – please try again."
                )
                return False

        except urllib.error.HTTPError as e:
            print(f"\n✗ HTTP error {e.code} while downloading: {e.reason}")
            print(
                "  Check your internet connection and that the model name "
                "is correct."
            )
            ...
            # Always clean up the temp file on any exit path
            try:
    ...
    def delete_model(model_name: str, force: bool = False) -> bool:
        """Delete a downloaded spaCy model from the user models directory.

        Models bundled inside the executable cannot be deleted.
        Models that were installed as Python packages (script mode) are also
        removed via pip so they no longer appear as installed.

        Args:
            model_name: Name of the model to delete
            force: If True, skip the confirmation prompt (used by the GUI)
        ...
        # Prevent deletion of the bundled default model
        if model_name == LexiFinderCLI.DEFAULT_MODEL and getattr(sys, 'frozen', False):
            ...
            print("  This model is part of the executable and cannot be removed.")
            return False

        if not LexiFinderCLI._is_model_available(model_name):
            print(f"\n✗ Model '{model_name}' is not installed.")
            return False

        # Confirmation prompt (CLI only – GUI passes force=True)
        if not force:
        ...
        # ── Remove from user models directory ────────────────────────────────
        models_dir      = LexiFinderCLI.get_models_directory()
        user_model_path = os.path.join(models_dir, model_name)
        if os.path.isdir(user_model_path):
            try:
                shutil.rmtree(user_model_path)
                print(f"✓ Removed: {user_model_path}")
                deleted_something = True
            except Exception as e:
                print(f"✗ Could not remove '{user_model_path}': {e}")
                return False

        # ── Also uninstall from Python packages when running as a script ─────
        # (only relevant if the user previously installed the model with pip)
        if not getattr(sys, 'frozen', False):
            ...
            if is_package:
                ...
                if proc.returncode == 0:
                    print(f"✓ pip package '{model_name}' uninstalled.")
                    deleted_something = True
        ...
        if deleted_something:
            ...
            print(f"✗ Nothing to delete for model '{model_name}'.")
            return False
    ...
    def list_installed_models():
        ...
        # Check for known models using the unified availability check
        for model_name in LexiFinderCLI.SUPPORTED_MODELS.keys():
            if LexiFinderCLI._is_model_available(model_name):
    ...
    def _resolve_model_path(model_name: str) -> str:
        ...
        Resolution order (frozen only):
        1. ``sys._MEIPASS/<model_name>``  – bundled with the exe at build time.
        2. ``~/.lexifinder_models/<model_name>`` – downloaded by the user at
           runtime via the Models tab or ``--download-model``.

        For non-frozen execution (normal Python interpreter) the model name
        is returned unchanged so that spaCy can locate it through its own
        ...
        """
        if getattr(sys, 'frozen', False):
            # 1. Bundled inside the executable (sys._MEIPASS)
            bundled_path = os.path.join(sys._MEIPASS, model_name)
            if os.path.isdir(bundled_path):
                return bundled_path

            # 2. Downloaded at runtime into the user models directory
            user_path = os.path.join(LexiFinderCLI.get_models_directory(), model_name)
            if os.path.isdir(user_path):
                return user_path
...
def run_gui():
    ...
    def main_gui(page: ft.Page):
        ...
        def _get_installed_models() -> list:
            """Return only the spaCy models that are currently available.

            Uses the unified ``_is_model_available`` check so that bundled and
            user-downloaded models are detected correctly in frozen executables.
            """
            return [
                m for m in LexiFinderCLI.SUPPORTED_MODELS
                if LexiFinderCLI._is_model_available(m)
            ]
        ...
        def _check_installed():
            """Return list of available models (bundled, downloaded, or installed)."""
            return [
                m for m in LexiFinderCLI.SUPPORTED_MODELS
                if LexiFinderCLI._is_model_available(m)
            ]
        ...
        def refresh_model_lists():
                    ...
                    def make_dl_btn(model_name):
                        def do_download(_=None):
                            def _run():
                                ...
                                async def _ui_refresh():
                                    refresh_model_lists()
                                page.run_task(_ui_refresh)
                            run_in_thread(_run)
                    ...
                    def make_del_btn(model_name):
                        def do_delete(_=None):
                            ...
                            def on_confirm(e):
                                ...
                                page.update()
                                def _run():
                                    # Redirect stdout → gui_logger so that
                                    # every print() inside delete_model()
                                    # appears in the GUI log panel.
                                    old_out, old_err = sys.stdout, sys.stderr
                                    sys.stdout = sys.stderr = gui_logger
                                    try:
                                        LexiFinderCLI.delete_model(model_name, force=True)
                                    except Exception as exc:
                                        log(f"\u2717 Error deleting model: {exc}")
                                    finally:
                                        sys.stdout, sys.stderr = old_out, old_err
                                    async def _ui_refresh():
                                        refresh_model_lists()
                            ...
                            dlg = ft.AlertDialog(
                                ...
                                actions=[
                                    ...
                                    ft.TextButton("Yes, delete", on_click=on_confirm),
                                ],
            ...
            # Also refresh the model dropdowns in Single File and Batch tabs
            def _make_opts(installed):
                if installed:
                    return [ft.DropdownOption(m, f"{m}  \u2013  {LexiFinderCLI.SUPPORTED_MODELS[m]}")
                            for m in installed]
                return [ft.DropdownOption("", "No models installed \u2013 go to Models tab")]

            _inst_now = _check_installed()
            _opts = _make_opts(_inst_now)
            _best = (_inst_now[0] if _inst_now else "")
            for dd in (sf_model, b_model):
                dd.options = _opts
                if dd.value not in _inst_now:
                    dd.value = _best
        ...
        _icon_src = (
            os.path.join(sys._MEIPASS, "lexifinder.svg")
            if getattr(sys, "frozen", False)
            else "lexifinder.svg"
        )
        _about_icon       = ft.Image(src=_icon_src, width=52, height=52)
...
if __name__ == "__main__":
    # GUI mode: double-click (no args) or explicit --gui flag
    # CLI mode: any other argument combination
