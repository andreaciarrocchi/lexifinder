...
### A note on the console window on Windows

Because `lexifinder.exe` supports both the graphical interface and the command-line interface from a single executable, it is built as a **console application**. This is a requirement of Windows: only console applications can write output directly to the terminal that launched them. As a result, a console window is always present alongside the graphical interface when LexiFinder is run in GUI mode — either as a visible window when launched by double-clicking, or as the terminal window you already have open when launched from the command line. This is expected behaviour and does not affect functionality in any way.

> **Why not hide the console window in GUI mode?** Windows does not allow a console application to cleanly detach from its own console window without side effects. The only way to avoid the console window entirely would be to build LexiFinder as a *windowed* application, but windowed applications on Windows cannot attach to an existing terminal: any CLI command would open a new, isolated console window, show its output for a moment, and then close — making the command-line interface effectively unusable. The visible console window in GUI mode is therefore an intentional trade-off, not a bug.

> **MSIX packages and the Microsoft Store**: if you install LexiFinder through an MSIX package or the Microsoft Store, the command-line interface will still work, but each command will open a new, separate console window rather than using the terminal you launched it from. This happens because MSIX uses a process activation broker that breaks the parent-process relationship the CLI relies on to attach to your existing terminal. It is an architectural constraint of the MSIX packaging format and not something LexiFinder can work around from within its own code. If you make extensive use of the command-line interface, it is recommended to download the standalone `lexifinder.exe` release from [GitHub](https://github.com/andreaciarrocchi/lexifinder) or [SourceForge](https://sourceforge.net/projects/lexifinder/) instead.
...
- **Feature requests** — suggestions for new features or improvements can be submitted as issues. Ideas currently under consideration include macOS release, and deeper integration with word processor index workflows.
